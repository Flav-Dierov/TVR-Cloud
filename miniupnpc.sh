...
update_subdomain() {
    ip=$(external-ip)
    if [ -z "$ip" ]; then
      echo "[ERROR] - $(date): Failed to retrieve external IP." >> update.log
      return
    fi

    curl "https://infomaniak.com/nic/update?hostname=$1$hostname&password=$password&username=$username&myip=$ip"
    echo "[INFO] - $(date): Sent update request for \"$1$hostname\" with IP \"$ip\"" >> update.log
}
...
check_server() {
    if ping -c 1 "$1"$hostname > /dev/null 2>&1; then
        echo "[SUCCESS] $(date): Ping to \"$1$hostname\" was successful." >> update.log
        if [ "$3" = false ]; then
            echo "[INFO] $(date): \"$1$hostname\" is back online." >> update.log
        fi
        set "$3" true
        set "$2" 0
    else
        (($2++))
        echo "[WARNING] $(date): \"$1$hostname\" couldn't be reached for $2 times" >> update.log
    fi
    if [ "$3" = false ]; then
        return
    elif [ "$2" -ge "$4" ]; then
        echo "[ALERT] $(date): \"$1$hostname\" couldn't be reached for $2 minutes. Taking action..." >> update.log
        $5
        set -- "$3" false
    fi
}
...
main_action() {
    echo "[ALERT] $(date): \"$subdomain_backup$hostname\" couldn't be reached for $fail_count_backup minutes." >> update.log
}
